...
export default function SignInPage() {
  ...
  useEffect(() => {
    ...
    if (params.get('oauth') === 'success' && params.get('token')) {
      try { saveSession({ token: params.get('token'), merchant: JSON.parse(decodeURIComponent(params.get('merchant') || '{}')) }); navigate('/dashboard', { replace: true }); } catch { setError('OAuth sign-in succeeded, but the session could not be restored.'); }
  ...
  async function submit(event) {
    ...
    setLoading(true);
    try { const { data } = await apiClient.post('/auth/login', form); saveSession(data); navigate('/dashboard'); } catch (requestError) { setError(requestError.response?.data?.message || 'Unable to sign in.'); } finally { setLoading(false); }
  }
