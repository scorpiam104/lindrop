import { ArrowRight, BarChart3, Bell, Check, ChevronDown, CircleDollarSign, ExternalLink, Facebook, Gauge, Github, LayoutDashboard, LogOut, Package, Plus, Search, Settings, Share2, ShieldCheck, Sparkles, Store, TrendingUp, UserCircle2, Users, X } from 'lucide-react';
...
export function DashboardLayout({ children }) {
  const navigate = useNavigate();
  const merchant = getMerchant();
  return <div className="dashboard-frame"><aside className="dashboard-sidebar"><div className="dashboard-brand"><span className="dashboard-brand-mark">+</span> flux</div><div className="dashboard-account"><div className="dashboard-avatar">{(merchant?.firstName || 'L').slice(0, 1).toUpperCase()}</div><div><strong>{merchant?.firstName || 'Lucas'} {merchant?.surname || 'Bennett'}</strong><span>{merchant?.email || 'merchant@linkpay.app'}</span></div><ChevronDown size={14} /></div><p className="dashboard-nav-label">Workspace</p><nav className="dashboard-nav">{navItems.map(([path, label, Icon]) => <Link key={path} to={path} className="dashboard-nav-link"><Icon size={16} /><span>{label}</span>{label === 'Overview' && <b>3</b>}</Link>)}</nav><div className="dashboard-upgrade"><span>Upgrade to Pro</span><small>Unlock premium tools for your store.</small><div className="upgrade-orbit" aria-hidden="true">✦</div><button onClick={() => navigate('/dashboard/store')}>Upgrade now</button></div><button onClick={() => { clearSession(); navigate('/login'); }} className="dashboard-signout"><LogOut size={15} /> Sign out</button></aside><main className="dashboard-main"><div className="dashboard-mobile-bar"><div className="dashboard-brand"><span className="dashboard-brand-mark">+</span> flux</div><button className="dashboard-icon-button" onClick={() => { clearSession(); navigate('/login'); }} aria-label="Sign out"><LogOut size={17} /></button></div>{children}</main></div>;
}
...
export function Overview() {
  ...
  return <DashboardLayout><Header title="Health overview" action={<><div className="dashboard-search"><Search size={16} /><input placeholder="Search..." aria-label="Search dashboard" /></div><button className="dashboard-icon-button" aria-label="Notifications"><Bell size={17} /><span>2</span></button></>} /><div className="dashboard-date-row"><p>Take control of your store today</p><span>12 July, 2024</span><button>Today <ChevronDown size={14} /></button></div><div className="dashboard-grid"><section className="dashboard-card sales-card"><CardHeading icon={<CircleDollarSign size={16} />} title="Sales overview" /><div className="sales-total"><strong>{formatGHS(sales).replace('GH₵', '₵')}</strong><span>+5.2%</span></div><p className="dashboard-muted">Total sales this month</p><div className="donut-chart"><div className="donut-value"><strong>{formatGHS(sales * .61).replace('GH₵', '₵')}</strong><span>Net sales</span></div><div className="donut-ring ring-purple" /><div className="donut-ring ring-ink" /><div className="donut-ring ring-lime" /></div><div className="sales-bars"><BarRow label="Online sales" value="45%" color="purple" /><BarRow label="Social orders" value="30%" color="ink" /><BarRow label="In-store" value="25%" color="lime" /></div></section><section className="dashboard-card metric-card"><CardHeading icon={<CircleDollarSign size={16} />} title="Revenue" /><strong className="metric-number">{formatGHS(sales * .42).replace('GH₵', '₵')}</strong><span className="metric-caption">+8.4% from last month</span><div className="mini-bars"><i /><i /><i /><i /><i /><i /><i /><i /><i /></div></section><section className="dashboard-card metric-card"><CardHeading icon={<TrendingUp size={16} />} title="Orders" /><strong className="metric-number">{stats.activeOrders || 0}</strong><span className="metric-caption">+12% this week</span><div className="sparkline"><i /><i /><i /><i /><i /><i /><i /></div></section><section className="dashboard-card wellness-card"><CardHeading icon={<Gauge size={16} />} title="Store health" /><strong className="metric-number">{Math.min(100, Math.round(Number(stats.conversionRate || 78)))}<small>%</small></strong><div className="dot-matrix">{Array.from({ length: 48 }, (_, index) => <i key={index} className={index % 5 === 0 || index % 7 === 0 ? 'active' : ''} />)}</div></section><section className="dashboard-card activity-card"><CardHeading icon={<BarChart3 size={16} />} title="Activity" /><div className="activity-value"><strong>{stats.totalViews || 0}</strong><span>views this week</span></div><div className="activity-chart">{[22, 38, 30, 52, 42, 68, 45, 78, 56, 70].map((height, index) => <i key={index} style={{ height: `${height}%` }} />)}</div></section><section className="dashboard-card orders-card"><div className="card-heading-row"><CardHeading icon={<Package size={16} />} title="Recent orders" /><Link to="/dashboard/orders">View all <ArrowRight size={14} /></Link></div><TransactionTable orders={orders.slice(0, 4)} /></section><section className="dashboard-card product-card"><div className="card-heading-row"><CardHeading icon={<Store size={16} />} title="Your products" /><Link to="/dashboard/products">View all <ArrowRight size={14} /></Link></div><div className="product-list">{featuredProducts.map((item, index) => <div className="product-row" key={item.title}><div className={`product-thumb product-thumb-${index}`}><span>{index === 0 ? '✦' : index === 1 ? '◒' : '▰'}</span></div><div><strong>{item.title}</strong><span>{item.tag}</span></div><b>{item.price}</b></div>)}</div><Link to="/dashboard/products" className="dashboard-primary-button"><Plus size={15} /> Add product</Link></section></div></DashboardLayout>;
}

function CardHeading({ icon, title }) { return <div className="card-heading"><span>{icon}</span><strong>{title}</strong><button aria-label={`${title} options`}>•••</button></div>; }
function BarRow({ label, value, color }) { return <div className="bar-row"><div><span>{label}</span><b>{value}</b></div><i className={color} style={{ width: value }} /></div>; }
...
export function StoreSetup() {
  ...
  return <DashboardLayout><Header title="Store setup" /><form onSubmit={submit} className="max-w-3xl rounded-[1.8rem] bg-white/90 p-6 shadow-[0_20px_60px_rgba(15,23,42,0.05)] ring-1 ring-slate-200 sm:p-8"><div className="grid gap-5 sm:grid-cols-2"><Field label="Business name" value={merchant?.businessName || ''} onChange={(e) => setMerchant({ ...merchant, businessName: e.target.value })} /><Field label="Custom URL slug" value={merchant?.slug || ''} onChange={(e) => setMerchant({ ...merchant, slug: e.target.value })} /><Field label="Store logo URL" value={merchant?.logoUrl || ''} onChange={(e) => setMerchant({ ...merchant, logoUrl: e.target.value })} /><Field label="Paystack public key" value={merchant?.paystackPublicKey || ''} onChange={(e) => setMerchant({ ...merchant, paystackPublicKey: e.target.value })} /></div><div className="mt-8"><p className="text-sm font-black text-slate-900">Customer storefront theme</p><p className="mt-1 text-sm text-slate-500">Choose how your public store feels to shoppers.</p><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">{storeThemes.map(([value, label, description]) => <button type="button" key={value} onClick={() => setMerchant({ ...merchant, storeTheme: value })} className={`rounded-2xl border p-3 text-left transition hover:-translate-y-0.5 ${merchant?.storeTheme === value ? 'border-cyan-400 bg-cyan-50 ring-2 ring-cyan-200' : 'border-slate-200 bg-white'}`}><span className={`block h-10 rounded-xl theme-preview-${value}`} /><span className="mt-3 block text-xs font-black text-slate-900">{label}</span><span className="mt-1 block text-[10px] leading-4 text-slate-500">{description}</span></button>)}</div></div><p className="mt-6 rounded-2xl bg-emerald-50 p-4 text-sm text-emerald-800 ring-1 ring-emerald-100">Your storefront: /store/{merchant?.slug}</p><button className="mt-6 rounded-2xl bg-slate-900 px-5 py-3 font-black text-white shadow-lg shadow-slate-200 transition hover:-translate-y-0.5 hover:bg-emerald-500 hover:text-slate-900">{saved ? 'Saved' : 'Save store settings'}</button></form></DashboardLayout>;
}
